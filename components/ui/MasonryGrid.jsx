...
export default function MasonryGrid({
  ...
  const columnedPhotos = useMemo(() => {
    ...
    const gapSize = 28; // gap-7 = 1.75rem = 28px

    photos.forEach((photo) => {
      const shortestColIndex = colHeights.indexOf(Math.min(...colHeights));
      cols[shortestColIndex].push(photo);
      colHeights[shortestColIndex] += (photoHeights[photo.id] || 300) + gapSize;
    });
  ...
  return (
    <div className="flex gap-2 md:gap-4">
      {columnedPhotos.map((columnPhotos, columnIndex) => (
        <div key={columnIndex} className="flex-1 flex flex-col gap-7">
...
const MasonryItem = React.memo(
        ...
        >
          <div className="relative">
            <motion.div
              ...
              onMouseEnter={() =>
                !isMobile && !isDeleteMode && setIsHovered(true)
              }
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      prevProps.photo.favoritedBy?.length ===
        nextProps.photo.favoritedBy?.length &&
